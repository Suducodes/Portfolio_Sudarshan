...
/* one project as a frosted-glass card (shared by orbit + mobile list) */
function Card({ p }) {
  return (
    <>
      <figure
      ...
      >
        <div className="relative aspect-[4/3] w-full overflow-hidden rounded-lg bg-void/60">
          ...
          <div
            ...
            style={{ background: `radial-gradient(130% 100% at 50% 120%, ${p.color}24, transparent 55%)` }}
          />
      ...
      <div className="mt-5 flex items-baseline justify-between gap-3">
        <div className="flex items-baseline gap-4 sm:gap-7">
          ...
          <h3 className="whitespace-nowrap font-display text-lg font-500 uppercase tracking-tight text-bone sm:text-2xl">
            {p.title}
          </h3>
        </div>
...
export default function Works({ onOpen }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const current = prog * (N - 1) * STEP
      if (ringRef.current) ringRef.current.style.transform = `rotateX(-7deg) rotateY(${-current}deg)`
      slots.current.forEach((el, i) => {
        if (!el) return
        let a = (((i * STEP - current) % 360) + 540) % 360 - 180 // -180..180
        const f = Math.max(0, 1 - Math.abs(a) / 95)
        el.style.opacity = (0.08 + 0.92 * f).toFixed(3)
        el.style.filter = `blur(${((1 - f) * 4).toFixed(1)}px)`
        el.style.zIndex = String(Math.round(f * 100))
        el.style.pointerEvents = f > 0.55 ? 'auto' : 'none'
      })
    }
    const onScroll = () => {
  ...
  return (
    <section id="work" ref={sectionRef} className="relative" style={{ height: `${(N + 1) * 90}vh` }}>
      <div className="pointer-events-none absolute inset-0 bg-void/45" />
      <div className="sticky top-0 flex h-screen w-full items-center justify-center overflow-hidden">
        {/* heading */}
        <div className="pointer-events-none absolute left-1/2 top-[12%] z-[200] -translate-x-1/2 text-center">
          <p className="mb-3 font-body text-[11px] uppercase tracking-[0.45em] text-teal/70">Selected Work — orbit to explore</p>
        ...
        {/* 3D orbit ring around the centered heart */}
          ...
          >
            {projects.map((p, i) => (
              <div
                ...
                onClick={() => onOpen(i)}
                className="group/card absolute w-[clamp(280px,26vw,400px)] cursor-pointer"
                style={{
                  left: '50%',
                  top: '50%',
                  marginLeft: 'calc(clamp(280px,26vw,400px) / -2)',
                  marginTop: '-150px',
